...
function get_reaction_rate(lamb::Float64, T::Float64)::Float64
  B = 10.0
  EbyR = 10000.0

  return B * (1-labm) * exp(-EbyR/T)
end
...
function solve(cfg::Config; filename::String="movie.mp4", fps::Int=30)
  ...
  rhs_func = make_rhs_func(MUSCL_minmod, cfg, i_start, i_end)
  ...
  while t < cfg.t_max
    ...
    U_arr = RungeKutta.rk_step(rhs_func, t, U_arr, dt, tab)
    apply_bc!(U_arr, cfg, i_start, i_end)
    ...
    if step % cfg.out_interval == 0
      ...
      Qs = compute_total_conserved(U_arr, cfg.dx, i_start, i_end)
